...
class CramMasterAgentXUI {
        ...
        if (!existingVisualizer) {
            const visualizerHTML = `
                <div id="stepVisualizer" class="step-visualizer hidden">
                    <h3>🚀 AgentX Workflow Progress</h3>
    ...
    initializeEventListeners() {
        ...
        // Generate Study Plan button - check both IDs
        const generateBtn = document.getElementById('generatePlan') || document.getElementById('generateBtn');
        if (generateBtn) {
            generateBtn.addEventListener('click', this.generateStudyPlan.bind(this));
        }

        // Run Full Demo button - check both IDs
        const runDemoBtn = document.getElementById('runFullDemo') || document.getElementById('runDemoBtn');
        if (runDemoBtn) {
    ...
    async generateStudyPlan() {
        ...
        if (!syllabusText || !syllabusText.value.trim()) {
            this.showNotification('Please provide syllabus content or upload a file! 📝', 'error');
            return;
        }

        const studyHours = parseInt(studyTime?.value || '0');
        if (!studyHours || studyHours < 1) {
            this.showNotification('Please specify valid study time! ⏰', 'error');
            return;
        }
        ...
        try {
            ...
            this.showNotification('🎉 Multi-Step AgentX Workflow Complete!', 'success');
    ...
    showWorkflowSteps() {
        const steps = [
            { id: 1, title: "📄 Processing Syllabus", description: "Analyzing uploaded content" },
            { id: 2, title: "🧠 AI Planning", description: "Generating personalized study roadmap" },
            { id: 3, title: "📝 Creating Quizzes", description: "Building interactive questions" },
            { id: 4, title: "⏰ Setting Timelines", description: "Optimizing study schedule" },
            { id: 5, title: "🎯 Finalizing Plan", description: "Preparing complete study experience" }
        ];
            ...
            <div class="agentx-results">
                        ...
                        <div class="topic-card enhanced" data-topic-id="${index}">
                            ...
                            <div class="topic-content">
                                ...
                                <div class="topic-meta">
                                    <span class="estimated-time">⏱️ ${topic.estimated_time || 30} min</span>
                                    <span class="quiz-count">❓ ${topic.quiz_questions?.length || 0} questions</span>
                                </div>
                            </div>
                            <div class="topic-actions">
                                <button class="btn-quiz" onclick="startQuiz(${index})">Start Quiz</button>
                                <button class="btn-hints" onclick="showHints(${index})">Study Hints</button>
                            </div>
                        </div>
                    `).join('')}
                </div>
                ...
                <div class="demo-features">
                    ...
                    <div class="feature-buttons">
                        <button class="btn-feature" onclick="ui.exportPlan('json')">📄 Export JSON</button>
                        <button class="btn-feature" onclick="ui.exportPlan('pdf')">📋 Export PDF</button>
                        <button class="btn-feature" onclick="ui.shareProgress()">🔗 Share Progress</button>
                        <button class="btn-feature" onclick="ui.startPomodoro()">🍅 Pomodoro Timer</button>
                    </div>
                </div>
            </div>
        `;
    ...
    // Full Demo Function
    async runFullDemo() {
        this.showNotification('🎯 Starting Full Demo Experience!', 'info');
        ...
        setTimeout(async () => {
            // Step 2: Start Timer
            this.showNotification('⏰ Starting Pomodoro Timer...', 'info');
            await this.startTimer();
            
            // Step 3: Trigger Quiz after 5 seconds (demo speed)
            setTimeout(() => {
                this.showNotification('📝 Launching Quiz...', 'info');
            ...
            setTimeout(() => {
                this.showNotification('📊 Opening Dashboard...', 'info');
                    ...
                    <div class="history-item">
                        <div class="history-item-header">
                            <span class="history-item-title">
                                ${session.completed ? '✅ Completed Session' : '⏸️ Incomplete Session'}
                            </span>
        ...
        quizModal.innerHTML = `
            <div class="quiz-content">
                <div class="quiz-header">
                    <h3>📝 Quiz: ${this.currentQuiz.topic.topic_name || this.currentQuiz.topic.topic}</h3>
                    <button class="close-quiz" onclick="ui.closeQuizModal()">✕</button>
                </div>
                ...
                <div class="quiz-navigation">
                    <button class="btn-secondary" id="prevBtn" onclick="ui.previousQuestion()" style="display: none;">← Previous</button>
                    <button class="btn-primary" id="nextBtn" onclick="ui.nextQuestion()">Next →</button>
        ...
        hintsModal.innerHTML = `
            <div class="hints-content">
                <div class="hints-header">
                    <h3>💡 Study Hints: ${topic.topic_name || topic.topic}</h3>
                    <button class="close-hints" onclick="this.parentElement.parentElement.parentElement.remove()">✕</button>
                </div>
...
7. NoSQL Databases
   ...
   - Graph databases (Neo4j)`;
        ...
        this.showNotification('🎬 Running full demo with sample DBMS syllabus!', 'info');
    ...
    async exportToPdf() {
        ...
        try {
            ...
            this.showNotification('📄 PDF export opened in new window. Use Ctrl+P to save as PDF.', 'success');
        } catch (error) {
    ...
    shareProgress() {
        ...
        if (navigator.share) {
            navigator.share(shareData).then(() => {
                this.showNotification('📱 Study plan shared successfully!', 'success');
            }).catch(() => {
    ...
    fallbackShare(shareData) {
        ...
        navigator.clipboard.writeText(shareText).then(() => {
            this.showNotification('📋 Share content copied to clipboard!', 'success');
        }).catch(() => {
    ...
    async startPomodoro() {
        this.showNotification('🍅 Pomodoro timer integration coming soon!', 'info');
    }
    ...
    async completeTimer() {
        ...
        if (this.currentSession) {
            try {
                ...
                this.addLogEntry('Pomodoro completed! Great focus! 🎯', 'success');
            } catch (error) {
                this.addLogEntry('Session completed locally', 'info');
            }
        }
        
        this.showNotification('🎉 Pomodoro completed! Great work!', 'success');
    ...
    startStudying() {
        this.showNotification('🎓 Happy studying! Use the timer to stay focused.', 'success');
        this.startTimer();
    }
    ...
    createNewPlan() {
        ...
        this.showNotification('Ready to create a new study plan! 📚', 'info');
    }
...
document.addEventListener('DOMContentLoaded', () => {
    ...
    try {
        ...
        console.log('UI initialized successfully:', window.ui);
        
        // Force global assignment for debugging
        window.startQuiz = function(topicIndex) {
            console.log('Global startQuiz called with index:', topicIndex);
            if (window.ui && typeof window.ui.startQuiz === 'function') {
                window.ui.startQuiz(topicIndex);
            } else {
                console.error('UI not available for startQuiz');
            }
        };
        
        window.showHints = function(topicIndex) {
            console.log('Global showHints called with index:', topicIndex);
            if (window.ui && typeof window.ui.showHints === 'function') {
                window.ui.showHints(topicIndex);
            } else {
                console.error('UI not available for showHints');
            }
        };
        
        console.log('Global functions assigned:', { startQuiz: window.startQuiz, showHints: window.showHints });
    ...
    setTimeout(() => {
        // Check for both button IDs and add fallback listeners
        const generateBtn = document.getElementById('generateBtn') || document.getElementById('generatePlan');
        const runDemoBtn = document.getElementById('runDemoBtn') || document.getElementById('runFullDemo');
        
        console.log('Fallback check - Generate button:', generateBtn);
        console.log('Fallback check - Demo button:', runDemoBtn);
        
        if (generateBtn && !generateBtn.hasAttribute('data-listener-added')) {
            console.log('Adding fallback event listener for generate button');
            generateBtn.addEventListener('click', function(e) {
                console.log('Fallback generate listener triggered');
                e.preventDefault();
                if (window.ui) {
        ...
        if (runDemoBtn && !runDemoBtn.hasAttribute('data-listener-added')) {
            console.log('Adding fallback event listener for demo button');
            runDemoBtn.addEventListener('click', function(e) {
                console.log('Fallback demo listener triggered');
                e.preventDefault();
                if (window.ui) {
                    window.ui.runFullDemo();
                }
            });
            runDemoBtn.setAttribute('data-listener-added', 'true');
        }
    }, 100);
});

// Old functions removed - quiz-system.js handles these now
